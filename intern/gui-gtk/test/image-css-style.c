 ...
 * certain events related to images.
 *
 * Some styling is done using CSS.
 */
...
/**
 * CSS style provider for the image.
 */
GtkCssProvider *image_css = NULL;
                    ...
                    GtkGrid *grid)
{
	...
	if (image_css != NULL)
	{
		gint start_grid_left = 0, start_grid_top = 0;
		get_grid_left_top (drag_start_x, drag_start_y, &start_grid_left, &start_grid_top);

		widget_at_start = gtk_grid_get_child_at (grid, start_grid_left, start_grid_top);

		g_assert (widget_at_start != NULL);

		gtk_style_context_add_provider (gtk_widget_get_style_context(widget_at_start),
		                                GTK_STYLE_PROVIDER (image_css), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
	}
}
              ...
              gpointer user_data)
{
	...
	GFile *common_styles_file;
	GtkCssProvider *common_styles;
	GError *wq_image_buf_load_error = NULL, *bh_image_buf_load_error = NULL;
	GError *common_styles_error = NULL, *image_styles_error = NULL;
	static const char *white_queen_path = "images/white_queen(106x106).bmp",
	                  *black_horse_path = "images/black_horse(106x106).bmp";
	static const char *image_css_path = "image-style.css";
	static const char *common_styles_path = "common-styles.css";
	...
	common_styles_file = g_file_new_for_path (common_styles_path);
	common_styles = gtk_css_provider_new ();
	gtk_css_provider_load_from_file (common_styles, common_styles_file, &common_styles_error);

	if (common_styles_error != NULL)
	{
		g_print ("Laoding common styles failed with error: %s\n", common_styles_error->message);
		g_error_free (common_styles_error);
	}
	else
	{
		gtk_style_context_add_provider (gtk_widget_get_style_context (window),
	                                        GTK_STYLE_PROVIDER (common_styles), GTK_STYLE_PROVIDER_PRIORITY_APPLICATION);
	}
	...
	/* Give some spacing between the grid cells */
	...
	if (wq_image_buf_load_error == NULL || bh_image_buf_load_error == NULL)
	{
		...
		g_signal_connect (grid_drag_gesture, "drag-begin", G_CALLBACK (drag_begin_cb), GTK_GRID (grid));
		...
		/* Initialise the style provider for the image */
		GFile *image_css_file = g_file_new_for_path (image_css_path);
		image_css = gtk_css_provider_new();
		gtk_css_provider_load_from_file (image_css, image_css_file, &image_styles_error);
		g_object_unref (image_css_file);

		g_assert (image_styles_error == NULL);

		if (image_styles_error != NULL)
		{
			g_print ("Loading styles failed with error: %s\n", image_styles_error->message);
			g_print ("Falling back to no styles.\n");
			g_error_free (image_styles_error);

			// free the css provider
			g_object_unref (image_css);
			image_css = NULL;
		}
